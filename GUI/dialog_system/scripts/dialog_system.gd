...
@onready var portrait_sprite : DialogPortrait = $DialogUI/NpcPortrait01
@onready var dialog_progress_indicator : PanelContainer = $DialogUI/DialogProgressIndicator
@onready var dialog_progress_indicator_label : Label = $DialogUI/DialogProgressIndicator/Next
...
func _unhandled_input(event) -> void:
	if is_active == false:
		return
	if (
		event.is_action_pressed("interaction") or
...
func set_dialog_text(_d : DialogText) -> void:
	...
	text_length = content.get_total_character_count() # Doesnot include BBCode
	plain_text = content.get_parsed_text() # Doesnot include BBCode
